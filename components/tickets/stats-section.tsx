...
function StatsSection() {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-6 lg:mb-4">
      {!isLoading && !isError && (
        <>
          <TicketStatsCard title="Revenu Total" value={formatCFA(ticketsStats.totalRevenus)} variant="primary" />
      ...
      {isLoading && Array.from({ length: 4 }).map((_, index) => <TicketStatsSkeleton key={index} />)}
