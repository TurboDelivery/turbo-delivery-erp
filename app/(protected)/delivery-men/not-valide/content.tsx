...
export default function Content({ initialData }: ContentProps) {
    const { columns, renderCell, renderCols, data, fetchData, currentPage, isLoading, isError, reessayer } = useContentCtx({ initialData });
    ...
    return (
        <div className="w-full h-full pb-10 flex flex-1 flex-col gap-4">
            ...
            {/* L'echec remplace la zone de donnees : pose au-dessus, il cohabiterait
                avec « Aucun livreur » et l'ecran se contredirait. */}
            {isError ? (
                <EtatErreur quoi="les livreurs" onReessayer={reessayer} enCours={isLoading} />
            ) : (
                <>
