...
export default function Content({ data: initialData }: { data: TraficLivreursResponse }) {
  ...
  useEffect(() => {
    const combinedPositions = [...data.disponibles.liste, ...data.enActivite.liste].filter((l) => l.position.latitude !== 0 && l.position.longitude !== 0);
  ...
  const DashboardPanel = () => (
    <motion.div key="dashboard" initial={{ opacity: 0, y: 100 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 100 }} transition={{ duration: 0.6, ease: 'easeOut' }} className="w-full">
      <Card className="w-full max-w-full sm:max-w-6xl mx-auto rounded-md">
        <CardHeader className="flex justify-center items-center pb-2">
          <Button size="sm" onClick={toggleDashboard} isIconOnly startContent={<ChevronDown />} variant="light" />
        </CardHeader>
        <CardBody className="space-y-6 px-4 sm:px-6 pb-6">
              ...
              {(() => {
                ...
                return (
                  <div className="text-sm space-y-1">
                    ...
                    {livreur.course && <p className="text-green-600">🚚 En course : {livreur.course}</p>}
                  </div>
                );
              })()}
            </div>
          ...
          <LivreurTimeline title={'Livreurs disponibles'} data={data.disponibles.liste} handleCourierSelect={handleLivreurSelect} />
          <LivreurTimeline title={'Livreurs indisponibles'} data={data.indisponibles.liste} handleCourierSelect={handleLivreurSelect} />
          <LivreurTimeline title={'Livreurs en activité de Livraison'} data={data.enActivite.liste} handleCourierSelect={handleLivreurSelect} />
        </CardBody>
      </Card>
    </motion.div>
  );
    ...
    <>
      ...
      <Card className="w-full h-[85vh] shadow-md overflow-visible relative">
        <CardHeader className="bg-primary text-white flex justify-between items-center px-4 py-3">
          ...
          <div className="text-sm font-medium bg-white text-primary px-3 py-1 rounded-md shadow">Total Livreurs: {data.totalLivreurs}</div>
        </CardHeader>
      ...
      <div className="relative">
        <AnimatePresence mode="wait">
          {!openDashboard ? (
            ...
            >
              <Button onClick={toggleDashboard} isIconOnly startContent={<LayoutDashboard />} variant="bordered" color="primary" size="sm" />
            </motion.div>
          ) : (
            <motion.div
              key="dashboard-panel"
