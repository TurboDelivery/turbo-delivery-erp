...
export default function useContentCtx({ initialData, restaurants }: Props) {
    ...
    const handleDateChange = (value: RangeValue<CalendarDate>) => {
        if (value.start && value.end) {
            setDates(value);
            handlePageChange(0); // remise à la 1ère page
        } else {
            setDates(null);
        }
    };
        ...
        const fetchData = async () => {
            ...
            try {
                const newData = await getBonLivraisonAll(
                    0, // toujours fetch tout pour le filtrage local
                    1000, // grand nombre pour tout récupérer
                    { dates: { start: dates?.start?.toString() ?? '', end: dates?.end?.toString() ?? '' } }
                );
                setData(newData);
            } catch (error) {
