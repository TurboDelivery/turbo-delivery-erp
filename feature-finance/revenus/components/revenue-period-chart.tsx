        ...
        {dateRange && (
          <Card className="p-4 bg-gray-50 border-gray-200">
            ...
            <div className="text-xs text-gray-600 space-y-1">
              ...
              <p><strong>Date début:</strong> {dateRange.from?.toISOString().split('T')[0]}</p>
              <p><strong>Date fin:</strong> {dateRange.to?.toISOString().split('T')[0]}</p>
              <p><strong>Jours:</strong> {dateRange.from && dateRange.to ? Math.ceil((dateRange.to.getTime() - dateRange.from.getTime()) / (1000 * 60 * 60 * 24)) : 0}</p>
            </div>
          </Card>
        )}
      </div>
