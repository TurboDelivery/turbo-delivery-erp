...
import { useEmployeeListQuery } from '../queries/employee-list.query';
...
export const useEmployeeTable = (externalFilters?: EmployeeFilters) => {
  ...
  const filteredEmployees = useMemo(() => {
    ...
    return filtered;
  }, [employees, currentFilters.departments, currentFilters.statuts, currentFilters.postes]);
  ...
  }, [filteredEmployees, currentFilters?.page, currentFilters?.limit, externalFilters, setFilters]);

  // Utiliser la pagination filtrée si filtres locaux, sinon pagination API
  const pagination = (currentFilters.departments || currentFilters.statuts || currentFilters.postes) 
    ? filteredPagination 
    : {
  ...
  const syncedSetFilters = (fn: (prev: EmployeeFilters) => EmployeeFilters) => {
    const newFilters = fn(filters as unknown as EmployeeFilters);
  ...
  const table = useReactTable({
    ...
    data: (currentFilters.departments || currentFilters.statuts || currentFilters.postes) ? paginatedFilteredEmployees : employees,
  ...
  return {
    ...
    employees: (currentFilters.departments || currentFilters.statuts || currentFilters.postes) ? paginatedFilteredEmployees : employees,
    employeesData,
    error,
