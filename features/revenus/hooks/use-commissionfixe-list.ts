...
import { useCommissionFixeListQuery } from "../queries/commission/commissionfixe-list.query";

export interface IUseCommissionFixeListProps {
    initialData?: ICommission[];
}

export function useCommissionFixeList({ initialData = [] }: IUseCommissionFixeListProps = {}) {
    ...
    const { data, isLoading, isError, error, isFetching } = useCommissionFixeListQuery(currentSearchParams);

    const commissionsfixe = data || initialData;
