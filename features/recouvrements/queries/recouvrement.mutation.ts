...
export const useModifierRecouvrementMutation = () => {
  ...
  return useMutation({
    mutationFn: async ({ id, data, factureDetails }: RecouvrementModificationData) => {
      ...
      const dto: Record<string, unknown> = {
        montant: data.montant,
        dateRecouvrement: data.dateRecouvrement.toISOString().split('T')[0],
        restaurantId: factureDetails.id,
      ...
      const result = await modifierRecouvrementAction(id, formData);
