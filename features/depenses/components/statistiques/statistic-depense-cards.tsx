...
export default function StatisticDepenseCards({ filters }: StatisticDepenseCardsProps) {
  ...
  // Utiliser les stats de l'API (non filtr̩es par cat̩gories pour le moment)
  const { data: statsData, isLoading: statsLoading } = useDepenseStatsQuery(currentSearchParams);

  // R̩cup̩rer toutes les d̩penses pour filtrer localement
  const depensesParams: IDepensesParams = {
    page: 0,
    limit: 1000, // Beaucoup pour avoir toutes les donn̩es
    debut: filters.debut,
    fin: filters.fin,
  };
  const { data: depensesData, isLoading: depensesLoading } = useDepensesListQuery(depensesParams);

  // Filtrer localement par cat̩gories si n̩cessaire
  const filteredDepenses = depensesData?.content?.filter(depense => {
    if (!filters.categoriesDepense || filters.categoriesDepense.length === 0) {
      return true; // Pas de filtre de cat̩gories
    }
  ...
  // Utiliser les stats locales si filtre de cat̩gories, sinon utiliser les stats API
  ...
  return (
    <div className="w-full">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <StatisticDepenseCard isLoading={isLoading} title="Cat̩gories" value={displayStats?.nombre_categories} color="text-blue-600" bgColor="bg-blue-100" icon={<Boxes className="h-5 w-5" />} />
        <StatisticDepenseCard
          isLoading={isLoading}
          title="Nombre de d̩pense"
