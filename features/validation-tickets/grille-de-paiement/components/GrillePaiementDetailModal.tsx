  ...
  return (
    <div className="rounded-2xl border border-[#d9d9d9] bg-[#f8f8f8] px-4 py-3">
      ...
      <div className="mt-2 flex items-end gap-1">
        ...
        {suffix && (
          <span className="mb-[3px] text-[10px] font-medium uppercase text-[#9b9b9b]">{suffix}</span>
        )}
      </div>
    </div>
  );
  ...
  return (
    <div className="flex items-start justify-between rounded-2xl border border-[#e6e6e6] bg-[#fcfcfc] px-3 py-3">
      <div className="flex gap-3">
        <div className={`mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-xl ${iconBg}`}>
          {icon}
        </div>
        <div>
        ...
        <div className="shrink-0 bg-[#f6f6f6] px-5 pt-5 pb-4 z-10">
          <div className="flex items-start justify-between">
            <div className="flex items-center gap-3">
              <div className="h-11 w-11 rounded-full bg-[#16a34a] text-white flex items-center justify-center font-semibold text-sm shadow-sm shrink-0">
                {initial}
              </div>
              <div>
          ...
          <div className="px-5 pb-6 flex flex-col gap-5">
            ...
            {bonusEligibilite && <div className="rounded-2xl border border-[#dddddd] bg-white p-4">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-[14px] font-semibold text-[#444444]">
              ...
              <div className="mt-4 space-y-3">
                {bonusEligibilite.criteres.map((critere, i) => (
                  <EligibilityItem
                    ...
                    icon={
                      i === 0
                        ? <ReceiptText size={15} className="text-white" strokeWidth={2} />
                        : <CircleAlert size={15} className="text-white" strokeWidth={2} />
                    }
                    iconBg="bg-[#111827]"
                    title={critere.label}
                    ...
                    rightIcon={
                      critere.valide
                        ? <CheckCircle2 size={17} className="text-[#22c55e]" strokeWidth={2} />
                        : <CircleAlert size={17} className="text-[#ef4444]" strokeWidth={2} />
                    }
                  />
                ))}
            ...
            {ticketDetails && <div>
              ...
              <div className="overflow-hidden rounded-2xl border border-[#dddddd] bg-white">
                ...
                {ticketDetails.length === 0 && (
                  <p className="px-4 py-6 text-center text-sm text-[#9a9a9a]">
                    Aucun ticket trouvé.
                  </p>
                )}
        ...
        <div className="shrink-0 bg-[#f6f6f6] px-5 pb-5 pt-0">
          ...
          <div className={`rounded-2xl border px-4 py-4 ${numeroWave ? 'border-[#b7e4c3] bg-[#dff7e7]' : 'border-[#fca5a5] bg-[#fef2f2]'}`}>
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-[10px] uppercase tracking-[0.18em] text-[#7f8f83]">
                  Numéro wave de paiement
                </p>
                <p className={`mt-1 text-[15px] font-medium ${numeroWave ? 'text-[#2d5a3f]' : 'italic text-[#ef4444]'}`}>
