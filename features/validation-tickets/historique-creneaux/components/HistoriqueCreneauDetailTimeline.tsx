...
> = {
  creation:   { icon: FileText,      iconClass: 'text-muted',  ringClass: 'bg-surface-secondary' },
...
export default function HistoriqueCreneauDetailTimeline({ events }: Props) {
  return (
    <div className="rounded-xl border border-separator bg-surface p-5 flex flex-col gap-4">
      <div>
        <h2 className="text-sm font-semibold text-foreground">Timeline du Créneau</h2>
            ...
            <div key={event.id} className="flex gap-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between gap-2">
                  <p className="text-sm font-semibold text-foreground leading-tight">{event.titre}</p>
                  <span className="text-[11px] text-muted shrink-0 whitespace-nowrap">{event.date}</span>
                </div>
                <div className="flex items-center gap-1.5 mt-0.5">
                  <span className="text-xs font-medium text-foreground">{event.acteurNom}</span>
                  <span className="text-[10px] text-muted border border-separator rounded px-1.5 py-px">{event.acteurRole}</span>
                </div>
                {event.commentaire && (
