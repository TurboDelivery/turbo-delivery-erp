...
export const useResponsableFinancierTable = (columns: ColumnDef<IFactureRF>[]) => {
  ...
  const params: IFactureRFParams = useMemo(() => ({
    periode: (filters.periode as IFactureRFParams['periode']) || undefined,
    dateDebut: filters.periode === 'plage' && filters.dateDebut
      ? filters.dateDebut.toISOString().split('T')[0]
      : undefined,
    dateFin: filters.periode === 'plage' && filters.dateFin
      ? filters.dateFin.toISOString().split('T')[0]
      : undefined,
